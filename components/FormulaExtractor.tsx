...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} className="p-1.5 rounded-lg bg-[#1a2340] border border-[rgba(245,200,66,0.1)] hover:border-[#f5c842]/30 transition-all text-[#8892a4] hover:text-[#f5c842]" title="Copy">
      {copied
...
function Section({
  ...
  return (
    <div className="border border-[rgba(245,200,66,0.1)] rounded-xl overflow-hidden">
      <button
        onClick={onToggle}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs px-2 py-0.5 bg-[#f5c842]/10 border border-[#f5c842]/20 rounded-full text-[#f5c842] font-mono">{count}</span>
        </div>
  ...
  if (!data && !loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-5 py-10">
        <div className="w-16 h-16 rounded-2xl bg-[#1a2340] border border-[rgba(245,200,66,0.2)] flex items-center justify-center text-3xl">🔬</div>
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4">
        ...
        <p className="text-[#f5c842] font-semibold text-sm">Scanning document...</p>
    ...
    <div className="flex flex-col h-full gap-3">
      ...
      <div className="relative shrink-0">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2 bg-[#0d1526] border border-[rgba(245,200,66,0.2)] rounded-xl text-[#f0f4ff] text-sm placeholder-[#8892a4] focus:outline-none focus:border-[#f5c842]"
        />
      </div>
            ...
            : d.formulas.map((f) => (
              <div key={f.id} className="p-3 bg-[#0d1526] border border-[rgba(79,195,247,0.15)] rounded-lg">
                ...
                <div className="flex gap-1.5 mt-2">
                  <button onClick={() => onExplain(f.content)} className="text-xs px-2.5 py-1 bg-[#f5c842]/10 border border-[#f5c842]/20 text-[#f5c842] rounded-lg hover:bg-[#f5c842]/20 transition-all">Explain</button>
                  <button
                    onClick={() => addToFlashcard(f.content, f.description ?? f.content, f.id)}
                    disabled={flashcardAdded.has(f.id)}
                    className={`text-xs px-2.5 py-1 rounded-lg border transition-all ${flashcardAdded.has(f.id) ? "bg-[#4ade80]/10 border-[#4ade80]/20 text-[#4ade80]" : "bg-[#1a2340] border-[rgba(245,200,66,0.1)] text-[#8892a4] hover:border-[#f5c842]/30"}`}
                  >
        ...
        <Section title="Key Definitions" icon="📖" count={d.definitions.length}
          ...
          {d.definitions.length === 0
            ...
            : d.definitions.map((def) => (
              <div key={def.id} className="p-3 bg-[#0d1526] border border-[rgba(245,200,66,0.12)] rounded-lg">
                <p className="text-[#f5c842] font-semibold text-sm">{def.term}</p>
                <p className="text-[#f0f4ff] text-xs mt-1 leading-relaxed">{def.definition}</p>
                <div className="flex gap-1.5 mt-2">
                  <button onClick={() => onExplain(`Explain the term "${def.term}"`)} className="text-xs px-2.5 py-1 bg-[#f5c842]/10 border border-[#f5c842]/20 text-[#f5c842] rounded-lg hover:bg-[#f5c842]/20 transition-all">Explain</button>
                  <button
                    onClick={() => addToFlashcard(def.term, def.definition, def.id)}
                    disabled={flashcardAdded.has(def.id)}
                    className={`text-xs px-2.5 py-1 rounded-lg border transition-all ${flashcardAdded.has(def.id) ? "bg-[#4ade80]/10 border-[#4ade80]/20 text-[#4ade80]" : "bg-[#1a2340] border-[rgba(245,200,66,0.1)] text-[#8892a4] hover:border-[#f5c842]/30"}`}
                  >
            ...
            : d.facts.map((f) => (
              <div key={f.id} className="p-3 bg-[#0d1526] border border-[rgba(79,195,247,0.1)] rounded-lg flex items-start gap-2">
                ...
                <div className="flex gap-1 shrink-0">
                  <button onClick={() => onExplain(f.content)} className="text-xs px-2 py-1 bg-[#f5c842]/10 border border-[#f5c842]/20 text-[#f5c842] rounded-lg hover:bg-[#f5c842]/20 transition-all">Explain</button>
                  <CopyBtn text={f.content} />
                </div>
              </div>
            ))}
      ...
      {/* Regenerate */}
      <button onClick={fetch_} className="shrink-0 flex items-center justify-center gap-2 py-2 bg-[#1a2340] border border-[rgba(245,200,66,0.15)] text-[#8892a4] text-xs rounded-xl hover:border-[#f5c842]/30 hover:text-[#f0f4ff] transition-all">
