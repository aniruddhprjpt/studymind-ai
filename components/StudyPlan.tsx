...
export default function StudyPlan({ documentContent }: { documentContent: string }) {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-full gap-4">
      ...
      <p className="text-[#c471ed] text-sm font-semibold">Building study plan...</p>
    </div>
  );
  ...
  if (!exam) return (
    <div className="flex flex-col items-center justify-center h-full gap-5 text-center px-6">
      ...
      <p className="text-[#8892a4] text-sm leading-relaxed">
        Click <span className="text-[#c471ed] font-semibold">Set Exam Date</span> in the top bar, then return here.
      </p>
    </div>
  );
    ...
    <div className="flex flex-col items-center justify-center h-full gap-5 text-center">
      ...
      <div>
        ...
        <p className="text-[#8892a4] text-sm mt-1 max-w-xs leading-relaxed">
          AI creates a day-by-day schedule for your <span className="text-[#c471ed]">{exam.name}</span> exam.
        </p>
      </div>
      {error && <p className="text-[#f87171] text-sm">{error}</p>}
      <button onClick={generate} className="flex items-center gap-2 px-5 py-2.5 bg-[#c471ed] text-[#fff] rounded-xl font-bold text-sm shadow-[0_0_20px_rgba(196,113,237,0.25)] hover:bg-[#d08cf0] transition-all">
  ...
  return (
    <div className="flex flex-col h-full gap-3">
      <div className="shrink-0 p-3 bg-[#0d1526] border border-[rgba(196,113,237,0.15)] rounded-xl">
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[#c471ed] text-xs font-semibold uppercase tracking-widest">{examName}</span>
        ...
        <div className="w-full bg-[#1a2340] rounded-full h-1.5">
          <div className="h-1.5 bg-gradient-to-r from-[#c471ed] to-[#4fc3f7] rounded-full transition-all" style={{ width: `${pct}%` }} />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin space-y-2">
        {plan.map((day) => {
          ...
          return (
            <div key={day.day} className={`border rounded-xl overflow-hidden transition-all ${done ? "border-[#4ade80]/30 bg-[#4ade80]/5" : "border-[rgba(196,113,237,0.12)] bg-[#0d1526]"}`}>
              <div className="flex items-center gap-3 px-3 py-2.5 cursor-pointer" onClick={() => setExpandedDay(exp ? null : day.day)}>
                <button onClick={(e) => { e.stopPropagation(); toggleDay(day.day); }}
                  className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${done ? "bg-[#4ade80] border-[#4ade80]" : "border-[#8892a4] hover:border-[#c471ed]"}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex gap-2"><span className="text-[#c471ed] text-xs font-bold font-mono">Day {day.day}</span><span className="text-[#8892a4] text-xs">{new Date(day.date).toLocaleDateString("en",{weekday:"short",month:"short",day:"numeric"})}</span></div>
              ...
              {exp && (
                <div className="px-3 pb-3 border-t border-[rgba(196,113,237,0.08)]">
                  <div className="flex flex-wrap gap-1.5 mt-2 mb-2">
                    {day.topics.map((t) => <span key={t} className="text-xs px-2 py-0.5 bg-[#c471ed]/10 border border-[#c471ed]/20 text-[#c471ed] rounded-full">{t}</span>)}
                  </div>
