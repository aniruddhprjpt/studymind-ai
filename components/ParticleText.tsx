...
const waitForFonts = async (font: string) => {
  ...
  try { await (document as Document & { fonts: FontFaceSet }).fonts.load(font); } catch { /* ignore */ }
...
export default function ParticleText({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      particles = selected.map((target, index) => {
        ...
        return {
          ...
          color: particleColor, seed, depth, delay: seed * stagger,
        };
      });
    ...
    const handleRMChange = (e: MediaQueryListEvent) => { reducedMotion = e.matches; sampleText(); };
    rmq?.addEventListener("change", handleRMChange);
    ...
    return () => {
      buildId += 1;
      ro.disconnect();
      rmq?.removeEventListener("change", handleRMChange);
